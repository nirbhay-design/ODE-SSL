...
def get_transforms(image_size, data_name = "cifar10", algo='supcon'):
    ...
    if algo == 'carl' or algo == "byol-sc" or algo == "simsiam-sc" or algo == "vicreg-sc":
        train_transforms = transforms.Compose([
            train_transforms,
            transforms.Normalize(mean = mean, std = std)
        ])
...
class DataCifar():
    ...
    def __getitem__(self, idx):
        ...
        if self.algo in ["nodel", "carl", "florel", "lema", "dailema", "scalre", "bt-sc", "simsiam-sc", "byol-sc", "vicreg-sc"]:
